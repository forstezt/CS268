var showFileName = true; // = true to show the filename in the popuped photo, = false to hide it
						 // Filenames in thumbnails and sizedPhotos folders must match each other							 
...
$(function() {
    ...
    $(".image").click(function() {
        ...
        if($("#popupphoto").is(":visible")) {
            fadeThenSlideIn(path);
        } else {
            slideIn(path);
        }        
    }); 
});
function fadeThenSlideIn(path) {
    $("#popupphoto").fadeOut(1000, function() {
