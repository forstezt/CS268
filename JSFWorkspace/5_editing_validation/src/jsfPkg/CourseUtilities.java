...
public class CourseUtilities implements Serializable {
	...
	public void runQuery(String action) {
		MySQL.open();
		try {
			//create the query statement
			String sqlQuery = "";
			if(action.equals("insert")) {
				...
				// execute the query
				pstmt.execute(); // no need to call retrieveCourses() here since addcourse will be redisplayed.
				...
				// the row from cData was assigned to c in the view c "is" the current row object
				// so setting this to false changes what is rendered in the view
				//c.setEditable(false);
				PreparedStatement pstmt = MySQL.cn.prepareStatement(sqlQuery);
				pstmt.setString(1, c.getC_callid());
				pstmt.setString(2, c.getC_name());
